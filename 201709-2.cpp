...
int getIndexById(int *array,int size,int id)
{
...
int main(int argc, char const *argv[])
{
    ...
    int (*data)[3]=(int(*)[3])malloc(sizeof(int)*k*3);

    int maxT=0; //记录程序结束时间
    for (int i = 0; i < k; i++)
    {
        cin>>data[i][0]>>data[i][1]>>data[i][2];
        maxT=data[i][1]+data[i][2]>maxT?data[i][1]+data[i][2]:maxT;
    }

   
    for(int i=1;i<=maxT;i++)   //i为时间
    {
        //处理还钥匙
        for (int j = 0; j < k; j++) //j二维数组的索引
        {
            if(i==data[j][1]+data[j][2])
            {
                for (int k = 0; k < n; k++) //寻找空位
                {
                    if(arr[k]==-1){
                        arr[k]=data[j][0];
                        break;
        ...
        //处理借钥匙
        for (int j = 0; j < k; j++) //j为钥匙编号
        {
            if(i==data[j][1])
            {
                int index=getIndexById(arr,n,data[j][0]);
                arr[index]=-1;
            }
        }
    }
