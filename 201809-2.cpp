...
bool isDuringPeriod(int (*arr)[2], int size, int time)
{
...
int main(int argc, char const *argv[])
{
    int n;
    cin >> n;
    int(*arr1)[2] = (int(*)[2])malloc(sizeof(int) * n * 2);
    int(*arr2)[2] = (int(*)[2])malloc(sizeof(int) * n * 2);
    ...
    int maxT = arr1[n - 1][1] > arr2[n - 1][1] ? arr1[n - 1][1] : arr2[n - 1][1];
    int sum = 0;
    for (int i = 1; i <= maxT; i++)
    {
        if (isDuringPeriod(arr1, n, i) && isDuringPeriod(arr2, n, i))
        {
            sum++;
        }
    }
